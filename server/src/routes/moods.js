...
// POST /api/moods/reminders - schedule a break reminder for this user
router.post('/reminders', validateBody(createReminderSchema), async (req, res) => {
  try {
    const { minutes, mood, energy } = req.validatedBody;
    const remindAt = new Date(Date.now() + minutes * 60 * 1000);
    const created = await prisma.breakReminder.create({
      data: {
        userId: req.user.id,
        minutes,
        remindAt,
    ...
    console.error('POST /api/moods/reminders failed:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});
